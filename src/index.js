...
const MAP_WIDTH = Math.floor(document.documentElement.clientWidth / (CONFIG.tileSize * SETTINGS.tileMag))
const MAP_HEIGHT = Math.floor(document.documentElement.clientHeight / (CONFIG.tileSize * SETTINGS.tileMag))
...
const generateMap = (width, height) => {
    ...
    let gen = new ROT.Map.Digger(width, height, {
        roomWidth: [3, 12],
        roomHeight: [4, 8],
        corridorLength: [3, 10],
        dugPercentage: 0.3,
...
document.body.onload = () => {
    ...
    console.log(display);
    display.getContainer().style.width = Math.floor(display._options.width * CONFIG.tileSize * SETTINGS.tileMag) + "px";
    display.getContainer().style.height = Math.floor(display._options.height * CONFIG.tileSize * SETTINGS.tileMag) + "px";
    generateMap(MAP_WIDTH, MAP_HEIGHT);
}

document.addEventListener("click", () => generateMap(MAP_WIDTH, MAP_HEIGHT));